...
// A witness function for the minimum signed distance, for two bodies, between
// all triangles *not already determined to be intersecting*. This witness
// function is to be activated when the bodies are sufficiently close as to
// pass the broad phase collision detection check. 
template <class T>
class SignedDistanceWitnessFunction : public systems::AbstractValues,
                                      public systems::WitnessFunction<T> {
 public:
  SignedDistanceWitnessFunction(
...
*/
 private:
  T DoEvaluate(const systems::Context<T>& context) const override {
    ...
    // The "bias" term to apply to the Euclidean distance, making it a pseudo-
    // signed distance. This tolerance should be modified to be based on the
    // relative size of the objects (using, e.g., bounding boxes). 
    const double kBias = 1e-8;
    ...
    return sqrt(signed_distance) - kBias;
  }
