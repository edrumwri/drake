...
TEST_F(KukaTest, ReachableTest) {
  ...
  if (gurobi_solver.available()) {
    ...
    const Body<double>& iiwa_link_2 =
        plant_->GetBodyByName(std::string_view("iiwa_link_2"));
    const Body<double>& iiwa_link_3 =
    ...
    EXPECT_NEAR(
        q_global_ik(
            plant_->GetJointByName(std::string_view("iiwa_joint_2")).index()),
        0.5, 1e-3);
    const JointIndex iiwa_joint_3_index =
        plant_->GetJointByName(std::string_view("iiwa_joint_3")).index();
    ...
    global_ik_.AddJointLimitConstraint(
        plant_->GetBodyByName(std::string_view("iiwa_link_4")).index(), 0.5,
        0.55);
    result = gurobi_solver.Solve(global_ik_.prog());
