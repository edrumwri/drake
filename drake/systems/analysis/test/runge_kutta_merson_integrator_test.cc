#include "drake/systems/analysis/runge_kutta_fehlberg_integrator.h"

// TODO(edrumwri): Remove this and fix corresponding build errors.
#include "drake/systems/analysis/runge_kutta_fehlberg_integrator-inl.h"
...
GTEST_TEST(IntegratorTest, MiscAPI) {
  ...
  RungeKuttaFehlbergIntegrator<double>
      integrator(spring_mass, context.get());
...
GTEST_TEST(IntegratorTest, ContextAccess) {
  ...
  // Create the integrator
  RungeKuttaFehlbergIntegrator<double> integrator(
...
GTEST_TEST(IntegratorTest, SpringMassStep) {
  ...
  integrator.set_error_control_norm(IntegratorBase<double>::eTwoNorm);
