#include "drake/systems/analysis/semi_implicit_euler_integrator.h"
...
GTEST_TEST(IntegratorTest, MiscAPI) {
  ...
  SemiImplicitEulerIntegrator<double> int_dbl(spring_mass_dbl, DT,
                                          context_dbl.get());
  SemiImplicitEulerIntegrator<AScalar> int_ad(spring_mass_ad, DT, context_ad.get());
...
GTEST_TEST(IntegratorTest, ContextAccess) {
  ...
  SemiImplicitEulerIntegrator<double> integrator(
