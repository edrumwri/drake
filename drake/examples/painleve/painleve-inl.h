...
template <class T>
Vector2<T> Painleve<T>::CalcRodEndpoint(const T& x, const T& y, const int k,
...
template <class T>
Vector2<T> Painleve<T>::CalcCoincidentRodPointVelocity(
...
template <class T>
T Painleve<T>::CalcMuStribeck(const T& us, const T& ud, const T& v) {
  T mu_dry;
  if (v >= 3)
    mu_dry = ud;  // sliding
  else if (v >= 1)
...
template <class T>
Vector3<T> Painleve<T>::CalcCompliantContactForces(
    const systems::Context<T>& context) const {
  using std::abs;
  using std::max;
  ...
  Vector2<T> p_WP[2] = {
      CalcRodEndpoint(p_WRo[0], p_WRo[1], -1, ctheta, stheta, half_rod_length),
      CalcRodEndpoint(p_WRo[0], p_WRo[1], 1, ctheta, stheta, half_rod_length)};
