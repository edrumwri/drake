...
output function, and an initial value:
...
system. The subscript indicates the values of these variables at step n, where
...
```
Analogous to a second-order ODE, we can write this second order system as a
...
class FibonacciDifferenceEquation : public systems::LeafSystem<double> {
 public:
  ...
  /** Allows an optional output stream for testing. */
  explicit FibonacciDifferenceEquation(std::ostream& out = std::cout)
      : out_(out) {
  ...
  // Output function yₙ = g(n, xₙ) to the output stream.
