...
GTEST_TEST(Radau3IntegratorTest, Robertson) {
  ...
  Radau3Integrator<double> integrator(*robertson, context.get());
...
TEST_F(ImplicitIntegratorTest, AutoDiff) {
  ...
  context->EnableCaching();
  Radau3Integrator<AutoDiffXd> integrator(*system, context.get());
...
TEST_P(ImplicitIntegratorTest, MiscAPI) {
  ...
  Radau3Integrator<double> integrator(*spring_, context_.get());
  ...
  EXPECT_EQ(integrator.get_jacobian_computation_scheme(),
            Radau3Integrator<double>::JacobianComputationScheme::
                kForwardDifference);
...
TEST_F(ImplicitIntegratorTest, FixedStepThrowsOnMultiStep) {
  ...
  // tolerances.
  Radau3Integrator<double> integrator(spring_mass, context_.get());
...
void CheckGeneralStatsValidity(Radau3Integrator<double>* integrator) {
...
TEST_P(ImplicitIntegratorTest, DoubleSpringMassDamper) {
  ...
  Radau3Integrator<double> integrator(*stiff_double_system_,
...
TEST_P(ImplicitIntegratorTest, SpringMassDamperStiff) {
  // Create the integrator.
  Radau3Integrator<double> integrator(*spring_damper_, context_.get());
  integrator.set_maximum_step_size(large_dt_);
  integrator.set_requested_minimum_step_size(small_dt_);
  integrator.set_throw_on_minimum_step_size_violation(false);
  integrator.set_reuse(GetParam());
...
TEST_P(ImplicitIntegratorTest, DiscontinuousSpringMassDamper) {
  // Create the integrator.
  Radau3Integrator<double> integrator(*mod_spring_damper_,
                                             context_.get());
  integrator.set_maximum_step_size(dt_);
  integrator.set_throw_on_minimum_step_size_violation(false);
  integrator.set_reuse(GetParam());
